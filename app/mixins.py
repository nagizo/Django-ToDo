...
class WeekCalendarMixin(BaseCalendarMixin):
    ...
    def get_week_days(self):
        ...
        for week in self._calendar.monthdatescalendar(date.year, date.month):
            if date in week:
                return week # 週ごとに取り出され、中身は全てdatetime.date型。該当の日が含まれていれば、それが今回表示すべき週
...
class WeekWithScheduleMixin(WeekCalendarMixin):
    ...
    def get_week_schedules(self, start, end, days):
        """それぞれのスケジュールを返す"""
        lookup = {
            #例）date__range: (1日, 31日)'を動的に作る
            '{}__range'.format(self.date_field): (start, end)
            }        
        # 例) Schedule.objects.filter(date__range=(1日, 31日)) になる
...
class MonthWithScheduleMixin(MonthCalendarMixin):
    ...
    def get_month_schedules(self, start, end, days):
        ...
        print(list(self.model.objects.all().values()))
